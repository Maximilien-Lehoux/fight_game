...
# Create characters
character1 = Characters(file_character1, location_character1, int(SPRITE_SIZE/1.8), int(SPRITE_SIZE*1.4))
character2 = Characters(file_character2, location_character2, int(SPRITE_SIZE/1.8), int(SPRITE_SIZE*1.4))
...
while not finish:
    ...
    level.display_decor(window, background_cloud, "c")
    level.display_decor(window, platform_wood, "w")
    window.blit(character1.picture, character1.rectangle)
    window.blit(character2.picture, character2.rectangle)
    pygame.display.flip()
